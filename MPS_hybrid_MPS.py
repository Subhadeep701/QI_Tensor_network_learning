...
X = combined_df.drop(columns=['dl_brate']).iloc[1000:1300]
y = combined_df['dl_brate'].iloc[1000:1300]
...
model_bond3 = MPSWithDenseBond3(input_size, embedding_dim, bond_dim=4, output_dim=output_dim).to(device)
...
# Define the model with a single MPS layer
class SingleMPSModel(nn.Module):
    def __init__(self, input_size, embedding_dim, bond_dim, output_dim):
        super(SingleMPSModel, self).__init__()
        self.mps_layer = tk.models.MPSLayer(
            n_features=input_size + 1,
            in_dim=embedding_dim,
            out_dim=output_dim,
...
# Initialize the Single MPS model
single_mps_model = SingleMPSModel(input_size, embedding_dim, bond_dim=3, output_dim=output_dim).to(device)
single_mps_model.mps_layer.trace(
    torch.zeros(1, input_size, embedding_dim, device=device),
    inline_input=False,
    inline_mats=False,
    renormalize=False
...
# Define the optimizer for the single MPS model
optimizer_single_mps = optim.Adam(single_mps_model.parameters(), lr=learning_rate, weight_decay=weight_decay)

# List to store test losses for the single MPS model
test_loss_single_mps = []

# Train and evaluate the Single MPS model
for epoch in range(num_epochs):
    single_mps_model.train()
    for batch_idx, (data, targets) in enumerate(train_loader):
        data = data.to(device)
        targets = targets.to(device)

        # Forward pass
        predictions = single_mps_model(data)
        loss = criterion(predictions, targets)

        # Backward pass
        optimizer_single_mps.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(single_mps_model.parameters(), max_norm=1.0)
        optimizer_single_mps.step()

    # Evaluate on test set
    single_mps_model.eval()
    with torch.no_grad():
        test_loss = sum(criterion(single_mps_model(data.to(device)), targets.to(device)).item() for data, targets in test_loader) / len(test_loader)
    test_loss_single_mps.append(test_loss)

# Print parameter complexity for the Single MPS model
single_mps_params = count_parameters(single_mps_model)
print(f"Number of trainable parameters in Single MPS Model: {single_mps_params}")

# Plot the test losses for all models
plt.figure(figsize=(10, 6))
#plt.plot(range(start, num_epochs + 1, epoch_diff), test_loss_mps[start-1:num_epochs:epoch_diff], label="Hybrid MPS Model (bond dimension 2)", color='black', marker='o', linestyle='--')
plt.plot(range(start, num_epochs + 1, epoch_diff), test_loss_ffn[start-1:num_epochs:epoch_diff], label="FeedForward Neural Network", color='black', marker='s', linestyle='--')
plt.plot(range(start, num_epochs + 1, epoch_diff), test_loss_mps_bond3[start-1:num_epochs:epoch_diff], label="Hybrid MPS Model (bond dimension 4)", color='black', marker='x', linestyle='--')
plt.plot(range(start, num_epochs + 1, epoch_diff), test_loss_single_mps[start-1:num_epochs:epoch_diff], label="Single MPS Model",  color='black', marker='d', linestyle='--')
plt.xlabel("Epoch")
